...
parser.add_argument('--recursive', type=bool, default=True,
                    help=('Find .wavs in all subdirectories'))
...
def get_all_wav_filenames(paths: Sequence[str], recursive: bool) -> List[str]:
    ...
    for path in paths:
        filenames.extend(
            glob.glob(os.path.join(path, ext), recursive=recursive))
    return filenames
...
class WAVDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(
        ...
        samples = self.load_func(self.wavs[idx], None)
        return samples
...
def get_activations(files, model, batch_size=50, dims=128, device='cpu',
                    num_workers=1, recursive=True):
    ...
    dataset = WAVDataset(files, recursive, model._preprocess)
...
def calculate_activation_statistics(files, model, batch_size=50, dims=2048,
                                    device='cpu', num_workers=1, recursive=True):
...
def compute_statistics_of_path(path, model, batch_size, dims, device,
    ...
    if path.endswith('.npz'):
        ...
        m, s = calculate_activation_statistics(path, model, batch_size,
                                               dims, device, num_workers, recursive)

    return m, s


def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1, recursive=True):
